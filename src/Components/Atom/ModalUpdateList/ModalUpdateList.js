...
const ModalUpdateList = (id) => {
  ...
  const handleSave = () => {
    ...
    axios
      .put(
        `https://d4608642-6ba1-4fc1-b625-fc9fc8fcd476-00-qmjm9kj9xpab.picard.replit.dev/ListTheraphy/v1/update/${id.id}`,
        updatedData
      )
      .then((res) => {
  ...
  useEffect(() => {
    if (id.id) {
      axios
        .get(
          `https://d4608642-6ba1-4fc1-b625-fc9fc8fcd476-00-qmjm9kj9xpab.picard.replit.dev/ListTheraphy/v1/getById/${id.id}`
        )
        .then((res) => {
              ...
              return (
                <div key={index}>
                  ...
                  <div>
                    <Form.Group
                      className="mb-3"
                      controlId={`deskripsi${index}`}
                    >
                      <Form.Label>Deskripsi Paket</Form.Label>
                      <Form.Control
                        defaultValue={paket.deskripsi}
                        type="text"
                        placeholder={`Deskripsi Paket ${index + 1}`}
                        onChange={(e) => {
                          handlePaketDesk(index, e.target.value);
                        }}
                      />
                    </Form.Group>
                    ...
                    <Form.Group className="mb-3" controlId={`harga${index}`}>
                      ...
                      <Form.Control
                        ...
                        type="number"
                        placeholder={`Harga Paket ${index + 1}`}
                        onChange={(e) => {
                          handlePaketHarga(index, e.target.value);
                        }}
                      />
                    </Form.Group>
                  </div>
