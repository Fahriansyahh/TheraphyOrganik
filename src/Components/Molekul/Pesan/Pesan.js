  ...
  const handlePesan = () => {
    if (selectedTheraphy && dataList && dari && sampai) {
      if (selectedTheraphy !== "Pilihan") {
        if (checked) {
          const data = {
            Theraphy: selectedTheraphy,
            Paket: dataList.PilihanPaket,
            Dari: dari,
            Sampai: sampai,
            Deskripsi: dataList.deskripsi,
            Harga: dataList.harga,
            Komentar: komentar,
          };
        ...
        <Form noValidate validated={validated} onSubmit={handleSubmit}>
          <Row className="mb-3">
            {idUser ? (
              ...
              <Alert
                key={"danger"}
                variant={"danger"}
                className="m-1 p-1 mb-3 mt-1 mx-auto "
                style={{ width: "80%" }}
              >
                Anda Belum Login
              </Alert>
          ...
          <Row className="mb-3 d-flex align-items-center ">
            ...
            >
              ...
              <Form.Control
                type="text"
                value={`Rp :${dataList?.harga ? dataList?.harga : "0"}`}
                required
                style={{
              ...
              >
                {ListToApi.map((data) => {
                  return (
                    <ListGroup.Item
                      key={data._id}
                      className="m-0 p-0 List_thraphy_Pesan"
